...
SCOPE:
  ...
  * (b) residual alignment (resnet / upsampler skip paths) is FIFO-aligned to the
    lagged conv-branch output (non-causal case).

This is the DEFAULT LTX-2.3 VAE decode path. Any shape/parity mismatch raises so the
caller (``Ltx2Sampler``) falls back to the legacy tiled decode. Escape hatch:
``LTX2_VAE_CHUNKED_DECODE=0`` forces the legacy path. Debug:
