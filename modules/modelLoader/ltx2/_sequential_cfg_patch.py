...
The diffusers LTX2 pipeline runs CFG by concatenating uncond+cond into a single batch=2
transformer forward. At high resolution every transient activation buffer in the
transformer is doubled, which drives sampling peak VRAM well above ComfyUI's
sequential-CFG footprint at the same shape.

This module wraps ``LTX2VideoTransformer3DModel.forward`` so that, when a flag is set on
the instance, a batch=2 call is split into two sequential batch=1 forwards (uncond first,
cond second, matching the pipeline's downstream ``chunk(2)`` order). Activations from the
first pass free before the second runs. Engaged only via the ``sequential_cfg`` context
manager so training and non-CFG sampling paths are untouched.

``_BATCH_KWARGS`` verified against the current diffusers pin's
``LTX2VideoTransformer3DModel.forward()`` top-level signature (transformer_ltx2.py) --
every name listed is a real forward() parameter that carries a leading batch dimension.
"""
...
def _sequential_cfg_forward(self, *args, **kwargs):
    ...
    # Normalize to all-kwargs so we can split by name. The first positional, if any, is
    # hidden_states per the forward signature.
    if args:
        kwargs = dict(kwargs)
        kwargs["hidden_states"] = args[0]
        args = args[1:]
        # The forward signature only takes hidden_states positionally in practice (the
        # pipeline calls it all-kwargs), but guard against extras.
        if args:
            return _original_forward(self, *args, **kwargs)
    ...
    # return_dict=True would require reconstructing an output object; the pipeline always
    # passes False, so fall through if someone asks for True.
    ...
    # Both are tuples (return_dict=False). Concatenate each tensor element along the batch
    # dim so the pipeline's downstream chunk(2) sees the same layout it would have from a
    # single batched call.
