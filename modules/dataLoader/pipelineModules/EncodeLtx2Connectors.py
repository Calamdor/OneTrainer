...
class EncodeLtx2Connectors(
    ...
    Runs once per item during the text-caching pass so that predict() can consume the
    pre-projected video_emb / audio_emb / attn_mask directly, eliminating the connector
    forward call per training step.

    Input ``hidden_state_in_name`` must be the full-length (max_token_length) flattened
    Gemma3 embedding -- shape (seq_len, hidden_size * num_layers). This module must be
    placed BEFORE PruneMaskedTokens in the pipeline so that the sequence length is
    constant (max_token_length) for all items. The fixed length means the cached tensors
    need no per-batch re-padding.
    """
