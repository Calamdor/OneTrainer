...
from modules.dataLoader.pipelineModules.EncodeGemma3Text import EncodeGemma3Text
...
@factory.register(BaseDataLoader, ModelType.LTX_2_3)
class Ltx2BaseDataLoader(
    ...
    def _preparation_modules(self, config: TrainConfig, model: Ltx2Model):
        ...
        # Gemma3 uses left-padding for chat-style prompts. Configure the tokenizer before
        # tokenization so PruneMaskedTokens/PadMaskedTokens work with the right padding
        # positions.
        ...
        # Run connectors here -- before PruneMaskedTokens -- so every item has the same
        # fixed sequence length (max_token_length=1024). Cached outputs are consumed
        # directly in predict(), eliminating the per-step connector call.
        ...
        # Text encoder is never trained for LTX-2.3 -- always cache embeddings.
    ...
    def _cache_modules(self, config: TrainConfig, model: Ltx2Model, model_setup: BaseLtx2Setup):
        ...
        # Text encoder is never trained for LTX-2.3 -- always cache. Connector outputs are
        # fixed-length (max_token_length=1024) so no per-batch padding is required -- cache
        # them alongside text embeddings.
        text_split_names = [
