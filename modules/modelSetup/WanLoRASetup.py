...
def _apply_companion_lora_hooks(
    ...
    for mod_path, weights in modules.items():
        ...
        if is_lora:
            # Hold d/u as buffers on a small standalone Module so the parent
            # transformer's .to(device) (called via transformer_1_to / _2_to,
            # which explicitly moves handle[2]) carries them along. Registering
            # buffers directly on `m` works for device-following but confuses
            # inductor's min-cut partitioner when m is inside a compiled block.
            ...
            holder.register_buffer("down", down, persistent=False)
            holder.register_buffer("up", up, persistent=False)
            holder.eval()

            def _make_lora_patched_forward(orig_fwd, h, s):
                def patched_forward(x):
                    return orig_fwd(x) + F.linear(F.linear(x, h.down), h.up) * s
                return patched_forward

            orig_forward = m.forward
            m.forward = _make_lora_patched_forward(orig_forward, holder, scale)
        else:  # OFT
        ...
        handles.append((m, orig_forward, holder if is_lora else rot_module))
        applied += 1
