...
class ModelSamplerOutput:
    def __init__(
        ...
        self.fps = fps
        # Optional audio waveform muxed into the same container as a video file. Shape
        # convention accepted by save_sampler_output: (channels, samples).
        self.audio = audio
        ...
        if sampler_output.file_type == FileType.IMAGE:
            ...
            if isinstance(sampler_output.data, torch.Tensor):
                ...
                if len(video_tensor.shape) == 4:
                    ...
                    if has_audio:
                        # Delegate to diffusers' own AAC muxer (ships with the LTX2 pipeline
                        # package, shared with Cosmos3): handles AAC stream creation, channel
                        # layout, format resampling, and proper interleaving with the video
                        # stream -- reused rather than hand-rolled.
