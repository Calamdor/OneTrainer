...
class Ltx2ModelSaver(
    ...
    def __save_safetensors(
        ...
        # ORIGINAL_TRANSFORMER = transformer only, converted from diffusers naming back to
        # the native Lightricks checkpoint layout via the same rename table the LoRA saver
        # hooks use (checkpoint_diffusers_to_original() is inherited from BaseModel, built
        # from Ltx2Model.diffusers_to_original() + fusion_groups() -- LTX-2.3 never fuses
        # qkv, so this is just the rename body, no fusion pre-stage).
